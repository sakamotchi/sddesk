...
[F2 経路①: キーボード]
  ...
  tauriApi.openNewWindow()  ← F1 で tabbingIdentifier 付与済み
    ↓
  new WebviewWindow(label, { ..., tabbingIdentifier: 'SDDesk' })
...
[F3: localStorage 名前空間分離]
  ...
  const label = getCurrentWindow().label  // "main" or "window-..."
    ↓
  persist(..., { name: `sddesk-app-store:${label}` })
    ↓
...
#[cfg(target_os = "macos")]
let app_submenu = SubmenuBuilder::new(&app_handle, "SDDesk")
    .about(None)
    .separator()
    .services()
    .separator()
...
**`appStore.ts`**: persist `name` を `` `sddesk-app-store:${label}` `` に動的化 + 旧キーから main への 1 回限りマイグレーション。

**`contentStore.ts` / `terminalStore.ts`**: persist 不使用のため変更なし（状態はウィンドウローカルでメモリ保持）。

**`settingsStore.ts`**: 言語設定 `sddesk-language` はユーザー横断共有のまま維持（変更なし）。
...
describe('appStore persist key (F3)', () => {
  ...
  it('旧キーから main キーへマイグレーションする', async () => {
    ...
    localStorage.setItem('sddesk-app-store', oldValue)
    vi.resetModules()
    await import('./appStore')
    expect(localStorage.getItem('sddesk-app-store:main')).toBe(oldValue)
    expect(localStorage.getItem('sddesk-app-store')).toBeNull()
  })

  it('既に main キーが存在する場合は上書きしない', async () => {
    localStorage.setItem('sddesk-app-store', '{"old":true}')
    localStorage.setItem('sddesk-app-store:main', '{"new":true}')
    vi.resetModules()
    await import('./appStore')
    expect(localStorage.getItem('sddesk-app-store:main')).toBe('{"new":true}')
  })
})
```
...
| `⌘N` は textarea / input でも発火する | SDDesk は読み取り専用アプリで編集可能な input がほぼない。検索バーや rename 時の input は `⌘N` を使わない | input 時は preventDefault しない（実装コスト対効果で却下、必要なら tasklist で判断） |
