...
- [x] `name: 'sddesk-app-store'` → `name: \`sddesk-app-store:${label}\`` に変更
- [x] 既存の `partialize` / `storage`（replacer/reviver）は維持

### T-6: 旧キーマイグレーション（WBS F3-1 付随）

- [x] `appStore.ts` トップレベルで、`label === 'main'` のとき旧キー `sddesk-app-store` → 新キー `sddesk-app-store:main` へ 1 回限りコピー & 旧キー削除
...
- [x] 既存の `appStore.test.ts` で使用している `sddesk-app-store` 直接参照を `sddesk-app-store:main` に更新
