...
- **受け入れ条件**:
  - [ ] セクション見出し（`Commands` / `Templates`）が i18n で表示される
...
- **説明**: セクション見出し（`Commands` / `Templates`）とバッジラベル（`CMD` / `TPL`）を `ja.json` / `en.json` に追加する。Phase B で追加される 2 種別のキーは本 Phase では未追加（必要になった時点で追記）
- **受け入れ条件**:
