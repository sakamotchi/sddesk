...
Claude Code の hooks コマンドから SDDesk に通知を送るには、外部プロセスから Tauri アプリにメッセージを送る IPC が必要。localhost HTTP サーバーを Tauri のセットアップフックで起動し、hooks スクリプトが curl で POST するエンドポイントを提供する。
...
- **説明**: SDDesk がアクティブウィンドウの場合は macOS 通知を抑制する
- **受け入れ条件**:
  - [ ] SDDesk にフォーカスがある状態では通知が表示されない
  - [ ] SDDesk が背面にある状態では通知が表示される
