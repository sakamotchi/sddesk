...
- macOS の「システム設定 → 通知」で SDDesk の通知が許可されていること
...
1. アプリが起動している状態で、**SDDesk 以外のウィンドウにフォーカス**する
...
1. **SDDesk ウィンドウにフォーカス**した状態で
2. **別のターミナル**で以下を実行（SDDesk のフォーカスを外さないよう注意）:
   ```bash
   curl -s -X POST -H "Content-Type: application/json" \
