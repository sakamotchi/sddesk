...
SDDesk の統合ターミナルで OSC 9 経由の OS 通知が発火したとき、発火元のタブに「未読印」を付けてユーザーが戻ってきたときに視覚的に判別できるようにする。ユーザーがそのタブを実際に見た瞬間にマークを解除する。
...
- macOS Terminal.app / iTerm2 などではタブに未読インジケーター（ドット / 色変化）が付き、該当タブを見ると解除される挙動が一般的。SDDesk でも同等の体験を提供したい。
