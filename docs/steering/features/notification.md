...
Claude Code などの AI CLI ツールから、SDDesk アプリ外でも処理状況をユーザーに通知する機能。ウィンドウが非フォーカス状態でも OS のネイティブ通知で「許可待ち」「完了」「エラー」などを知らせる。
...
| N-09 | 設定永続化 | `~/.config/sddesk/config.json` の `appearance.notificationEnabled` に保存 |
...
`~/.config/sddesk/config.json` の `appearance` セクション:
